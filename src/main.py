...
advanced_blur_transforms = A.Compose([A.Resize(HEIGHT, WIDTH, p=1, always_apply=True), A.AdvancedBlur(), ToTensorV2()])
...
if DATASET == "new_plants":
    ...
    test_dataset_base_line = class_dataset_C.PlantImageDatasetC(csv_file=parse_yaml['csv'][DATASET]['test'], root_dir=parse_yaml['root_dir'][DATASET], main_dir=main_dir, transform=valid_transforms, albumentation_transform=baseline_transforms)
