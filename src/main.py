...
gauss_noise_transforms = GaussNoise(p=1.0, var_limit=(10, 50))
motion_blur_transforms = MotionBlur(p=1.0, blur_limit=7)
random_fog_transforms = RandomFog(p=1.0, fog_coef_lower=0.3, fog_coef_upper=1.0, alpha_coef=0.08)
random_rain_transforms = RandomRain(p=1.0, brightness_coefficient=0.9, drop_width=1, drop_color=(200, 200, 200), blur_value=5)
random_snow_transforms = RandomSnow(p=1.0, brightness_coeff=2.5, snow_point_lower=0.1, snow_point_upper=0.3)
random_sun_flare_transforms = RandomSunFlare(p=1.0, flare_roi=(0, 0, 1, 0.5), angle_lower=0, angle_upper=1, num_flare_circles_lower=6, num_flare_circles_upper=10, src_radius=300)
...
if DATASET == "new_plants":
    ...
    test_dataset_random_sun_flare = class_dataset_C.PlantImageDatasetC(csv_file=parse_yaml['csv'][DATASET]['test'], root_dir=parse_yaml['root_dir'][DATASET], main_dir=main_dir, albumentation_transform=random_sun_flare_transforms)
